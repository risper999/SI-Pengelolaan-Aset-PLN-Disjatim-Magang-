...
<body>
  <div class="" style="margin: 100px 70px 0 70px;">
    <div class="card">
      <h5 class="card-header text-white bg-primary"><i class="fas fa-stream"></i> Form Status Pendayagunaan</h5>
      <div class="card-body">
        <?php
          if($this->session->userdata('aksi')==true) {
            echo form_open('status_pendayagunaan/edit_simpan');
              echo form_hidden('no_id',$no_id['id_pendayagunaan']);
            }else{
              echo form_open('status_pendayagunaan/input_simpan');
            } ?>
        <div class="form-group">
          <label for="inputPassword4">Deskripsi*</label>
          <?php if($this->session->userdata('aksi')==true) {
              echo '<input type="text" class="form-control" name="desc_pendayagunaan" value="'.$no_id['deskripsi_pendayagunaan'].'" placeholder="-" required/>'; 
            }else{
              echo '<input type="text" class="form-control" name="desc_pendayagunaan" placeholder="-" required/>';    
            } ?>
        </div>
      </div>
